...
namespace ARPGLoot
{
    public class ARPGPlayer : ModPlayer
    {
        ...
        // onHit and onHitChance are now stored per item/projectile
        //public string onHit = "";
        ...
        public static Item swingingItem = null;
        public static bool swingingCheck = false;

        public override bool PreItemCheck()
        {
            this.PrepareShotDetection();
            return true;
        }

        public override void GetWeaponDamage(Item item, ref int damage)
        {
            this.BeginShotDetection(item);
        }

        public override void PostItemCheck()
        {
            this.EndShotDetection();
        }
        ...
        private void PrepareShotDetection()
        {
            ...
            {
                ARPGPlayer.swingingItem = player.HeldItem;
            }
        }

        private void BeginShotDetection(Item item)
        {
            if (ARPGPlayer.swingingItem == item)
            {
                ARPGPlayer.swingingCheck = true;
            }
        }

        private void EndShotDetection()
        {
            ARPGPlayer.swingingItem = null;
            ARPGPlayer.swingingCheck = false;
        }

        // This has been moved to ARPGProj
        /*
        ...
        */
        ...
        {
            Random rand = new Random();
            ARPGItem modItem = item.GetGlobalItem<ARPGItem>();

            int onHitChance = modItem.onHitChance;

            if (rand.Next(0, 100) + 1 <= onHitChance)
            {
                string onHit = modItem.onHit;
